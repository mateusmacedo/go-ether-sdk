...
import (
	...
	"github.com/mateusmacedo/go-ether-sdk/application/helpers/collection"
	"github.com/mateusmacedo/go-ether-sdk/application/logging/fixture"
)
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				go func() {
					...
					if got := processReverse(tt.args) ; !collection.SliceIsEqual(got, tt.want) {
						t.Errorf("LogMiddleware() = %v, want %v", got, tt.want)
					}
				}()
				<-done // Espera a goroutine terminar.
